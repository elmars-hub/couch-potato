    ...
    <main className="min-h-screen bg-[#141414] text-white">
      <div className="container mx-auto px-4 pt-28 pb-10">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {data.map((item) => (
              <WatchlistCard key={`${item.mediaType}-${item.mediaId}`} mediaType={item.mediaType} mediaId={item.mediaId} />
            ))}
          </div>
        ) : (
...
function WatchlistCard({ mediaType, mediaId }: { mediaType: "movie" | "tv"; mediaId: number }) {
